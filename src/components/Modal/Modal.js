...
export default function Modal({ActiveModal, SetActiveModal, offsetPadding}) {
  ...
  useEffect(()=> {
   ...
   function CheckoffsetPadding() { 
      if(ActiveModal) {
       // передаем в вызов AddOffsetPadding параметр 
  ...
  // используем e.stopPropagation() чтобы запреить срабатывание на самой форме а только на фоне  
  return (
 <>
...
{/* Блокируем закрытие Окна по клику на кнопке НО очищаем форму просто */}
